...
public interface ChallengeRepository extends JpaRepository<ChallengeEntity, Long> {
    ...
    @Query("select c from ChallengeEntity c where c.isPublic = true and c.status <> :status")
    ...
    @Query("select c from ChallengeEntity c where c.id = :challengeId and (c.isPublic = true or c.creatorId = :userId or c.id in (select cp.challengeId from ChallengeParticipantEntity cp where cp.userId = :userId and cp.participantStatus in ('INVITED','ACCEPTED')))")
